...
[NOTE]
====
*This is the Julia-library variant.* `rsr-julia-library-template-repo` is a
sibling of the generic `rsr-template-repo`, with the Rust-primary CI dropped
and Julia scaffolding added: `Project.toml` (closed `[compat]`), `src/Widget.jl`
(rename this — it's a throwaway example, not a real package), `test/runtests.jl`
+ `test/aqua.jl` + `test/jet.jl`, and Documenter docs (`docs/make.jl`). Every
library instantiated from this template is measured against
`docs/flagship-rubric.adoc`, the same rubric `Axiom.jl` scores 100% on as the
estate's flagship reference instance.
====
...
| `test/`                   | Julia Pkg test harness (`runtests.jl`, Aqua, JET) — what `Pkg.test()` runs.
| `tests/`, `benches/`      | RSR shell test suites (e2e, fuzz, workflow validation) and benchmarks — inherited estate harness, deliberately distinct from Julia's `test/`.
| `.machine_readable/descriptiles/VARIANT.a2ml` | Variant contract: how this template relates to `rsr-template-repo` (parent pin + declared path divergences, enforced by the Variant Drift Gate).
|===
